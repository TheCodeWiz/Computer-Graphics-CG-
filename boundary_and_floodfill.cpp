...
void FFill(int x, int y , int newcolor, int delay_time1)
{
	int currpixel= getpixel(x,y);
	if(currpixel==NULL)
	{
		putpixel(x,y,newcolor);
        delay(delay_time1);
		FFill(x-1,y,newcolor, delay_time1);
		FFill(x+1,y,newcolor, delay_time1);
		FFill(x,y+1,newcolor, delay_time1);
		FFill(x,y-1,newcolor, delay_time1);
	}
}
...
int main()
{
	...
	BFill(x/2+30,y/2-30,2, 1.1);

    setcolor(BLUE);
	line(x/2+70,y/2-10,x/2+70,y/2-70);
	setcolor(GREEN);
	line(x/2+70,y/2-70,x/2+130,y/2-70);
	setcolor(RED);
    line(x/2+130,y/2-10,x/2+130,y/2-70);
    setcolor(YELLOW);
    line(x/2+70,y/2-10,x/2+130,y/2-10);
    FFill(x/2+75,y/2-30,2, 1);
